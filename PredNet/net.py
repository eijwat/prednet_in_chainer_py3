...
class PredNet(chainer.Chain):
    ...
    def cal_eval_index(self, y):
        E = [None] * self.layers
        eval_index = [None] * self.layers
        for nth in range(self.layers):
            if nth == 0:
                A = y
                E[nth] = F.concat((F.relu(y - getattr(self, 'P' + str(nth))),
                                  F.relu(getattr(self, 'P' + str(nth)) - y)))
            else:
            ...
            eval_index[nth] = F.mean_squared_error(A, getattr(self, 'P' + str(nth)))
        return eval_index
